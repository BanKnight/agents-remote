...
- 2026-05-29：根据 mobile/desktop 原型复查修正 shared bottom navigation vertical 分支、一级/二级 navigation active 宽度契约、移动端响应式 shell/safe-area、Home mobile topbar 文案、Project mobile 直接顶部栏；Project 卡片缺省分支状态缩短为 `pending`；将导航形态、safe-area 与短状态 pill 共同约束回写 version shared 和 implement references；重新执行 typecheck、web tests、console-model tests、build 和 browser artifact capture，verify Round 5 通过。
- 2026-05-29：清理旧 browser capture 残留进程，并修复 `capture-home-project-shell.ts`：capture API/Web 以 detached process group 启动，finally 中终止整个进程组，避免 `bun run src/index.ts` 或 Vite 子进程 orphan 残留；复跑 capture 后确认只剩人工测试服务 `43011/43012`。
- 2026-05-29：根据 Project/Home 用色复查补齐横向与纵向 surface 抽象：`shellSurfaceClasses` 统一 shell/sidebar/workspace/header/bottom nav/raised/dashed/inset/code/danger/warning roles，Home/Project/Files/Git/Terminal 共享这些 role；同时将原型持续对照、横纵抽象、真实能力边界写入项目 `CLAUDE.md` 和 implement references。
