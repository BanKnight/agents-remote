...
本文件记录 `setup-e2e-quality-baseline` 对 `e2e-quality-baseline` 的行为契约增量。

## Change 来源

- change-id：setup-e2e-quality-baseline
- 来源意图：用户希望尽早建立自动化 E2E 测试，覆盖 `web + api + session runtime` 的核心联通；第一条 smoke/e2e 链路应覆盖登录、Project 列表、进入 Project、创建 Terminal Session、连接终端并看到可交互输出，且 Terminal 链路尽量使用真实 `tmux/shell` 与 WebSocket 交互。
- 规划来源：v0.3-session-runtime-quality 的质量基础设施 change；依赖已完成的 `design-session-runtime-boundaries`，并为后续持续集成、人工测试和移动端/PWA 验证提供基线。
