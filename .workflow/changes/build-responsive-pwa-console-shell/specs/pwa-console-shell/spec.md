...
本文件记录 `build-responsive-pwa-console-shell` 对 `pwa-console-shell` 的行为契约增量。

## Change 来源

- change-id：build-responsive-pwa-console-shell
- 来源意图：编号 1、30、31、41、42、43、75、76、77
- 规划来源：v0.2-project-console-shell 中的响应式 PWA 控制台外壳；在真实 Agent Runtime 接入前先完成可安装、移动端优先、深色控制台入口。
...
系统 SHALL 以移动端优先的深色控制台作为第一轮默认界面，不提供浅色主题切换作为本 change 的行为要求。
...
系统 SHALL 不把系统通知作为本 change 的首轮必备行为；第一轮优先保证用户打开网页或 PWA 后能快速观察运行中和等待输入的会话。
