...
- [x] 1.1 建立 spec 覆盖矩阵
  ...
  - 依赖：无。
  - 并行：否（阻塞后续任务）。
  - 结果：已创建 `artifacts/spec-coverage-matrix.md`，所有 requirement/scenario 均已映射到研究材料、design 文件或明确待验证项；当前无未映射 requirement。

### 2. 核心整理任务

- [x] 2.1 补齐 hapi/remodex 源码证据追溯
  ...
  - 结果：已创建 `artifacts/source-evidence.md`，校验 hapi/remodex 本地路径与 commit，补齐源码证据表，并修正 hapi 过期 provider 路径为实际存在的 AgentRegistry/runner/ACP backend 路径。

- [x] 2.2 收口官方资料与社区弱信号分级
  ...
  - 结果：已创建 `artifacts/evidence-grading.md`，按源码证据、官方资料、社区弱信号收口 Claude/Codex/官方 app/社区反馈的证据等级，并在研究主文档加入 artifact 指针。

- [x] 2.3 固化 V1 与 Final 边界的验证清单
  ...
  - 结果：已创建 `artifacts/verification-checklist.md`，覆盖 spec scenarios、V1 terminal passthrough 保真项、Final provider-native/capability-based 边界、non-freeze list 和开放验证问题。

- [x] 2.4 编写下游 change handoff
  ...
  - 依赖：2.1、2.2、2.3。
  - 并行：否（依赖核心整理结论）。
  - 结果：已创建 `artifacts/downstream-handoff.md`，分别为 session runtime、provider experience、E2E baseline 写出必须消费结论、禁止固化边界、开放问题和建议验证动作。

### 3. 集成与验证任务

- [x] 3.1 执行一致性检查并准备 verify 输入
  ...
  - 依赖：2.4。
  - 并行：否（最终收口任务）。
  - 结果：已确认必需产物存在、所有 tasks 已完成、`verification-checklist.md` 覆盖所有 spec scenarios，并更新 critical gap status 为无 unresolved critical gap。
