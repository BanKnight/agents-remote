...
本文件记录 `implement-agent-provider-experience` 对 `agent-provider-experience` 的行为契约增量。

## Change 来源

- change-id：implement-agent-provider-experience
- 来源意图：Claude/Codex 在界面上应明确显示为不同 Agent provider；provider 差异应由 Agent Runtime 吸收，控制面保持统一 Agent Session 语义；历史会话读取和恢复方向需要被记录清楚并分阶段推进。
- 规划来源：依赖已完成的 `research-agent-access-options` 与 `design-session-runtime-boundaries`，在稳定 Session Runtime 边界上补齐 provider 层体验。
...
- **AND** 不在本 change 范围内管理 provider 账号登录、安装或模型配置
