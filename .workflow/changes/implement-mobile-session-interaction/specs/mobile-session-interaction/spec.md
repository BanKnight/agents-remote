...
本文件记录 `implement-mobile-session-interaction` 对 `mobile-session-interaction` 的行为契约增量。

## Change 来源

- change-id：implement-mobile-session-interaction
- 来源意图：用户希望 Agent/Terminal 详情页在手机竖屏中真实可用：终端区域可读，底部区域优先服务多行辅助输入、快捷键和展开/收起，不使用底部 Tab 挤占交互空间；快捷键先以代码默认集合区分 Agent/Terminal，后续再考虑用户配置。
- 规划来源：依赖已完成的 `design-session-runtime-boundaries` 与 `build-responsive-pwa-console-shell`，在稳定 Session Runtime 和移动端 PWA shell 上补齐会话详情页移动端交互。
...
### Requirement: Default quick keys are provider/session-type aware without first-round configuration UI
