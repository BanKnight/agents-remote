...
- [ ] 1.1 创建根 Bun workspace 与基础 TypeScript 配置
  - 验收标准：
...
- [ ] 2.1 创建 `api` Bun 服务骨架与最小 `/api` smoke 能力
  - 验收标准：
...
- [ ] 2.2 创建 `web` React + TypeScript + Vite + Tailwind 基础
  - 验收标准：
...
- [ ] 2.3 接入 TanStack 与 Jotai 的前端基础边界
  - 验收标准：
...
- [ ] 3.1 补齐根脚本、基础测试/质量入口和 workspace 验证
  - 验收标准：
...
- [ ] 3.2 验证 `/api` HTTP 与 WebSocket dev proxy
  - 验收标准：
