...
本文件记录 `implement-git-diff-viewer` 对 `git-diff-viewer` 的行为契约增量。

## Change 来源

- change-id：implement-git-diff-viewer
- 来源意图：用户希望第一步 Git 功能只查看 Project 内已修改文件的 diff；先展示变更文件列表，点击单文件查看 unified diff；覆盖工作区和 staged 变更；非 Git 仓库要清晰提示；移动端优先使用 unified diff 文本；文件列表显示路径和 modified/added/deleted/renamed 等基本状态；不做 Git 写操作、复杂筛选或 PC 双栏 diff。
- 规划来源：v0.4-project-inspection-tools 的 Project 观察能力；依赖已完成的 Project model/safe paths、响应式 PWA Console Shell 和 Files 只读观察入口经验。

## ADDED Requirements

### Requirement: Git diff viewer is read-only

系统 SHALL 在 Project console 内提供只读 Git diff 观察能力，展示当前 Project 的 Git 变更文件和单文件 diff，但第一轮不提供任何 Git 写操作。
...
- **WHEN** 第一轮 Git 页面被评审
- **THEN** 不应存在可触发 Git 写操作的用户操作
- **AND** API 不应提供 Git 写操作作为本 change 的一部分
...
系统 SHALL 在第一轮变更文件列表中展示基本状态类型，例如 modified、added、deleted、renamed。
