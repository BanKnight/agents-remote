...
本文件记录单个 change 对 `project-safe-paths` 的行为契约增量。

## Change 来源

- change-id：implement-project-model-and-safe-paths
- 来源意图：4, 7, 33, 79, 80, 108
- 规划来源：Project 是文件、Git、Terminal 和 Agent Session 的统一作用域，路径安全必须集中实现。
...
系统 SHALL 将 `personal-app-config` 中已验证为绝对路径的 `PROJECTS_ROOT` 作为 Project 能力的根信任边界；本 change 不改变 `PROJECTS_ROOT` 的配置来源优先级。
