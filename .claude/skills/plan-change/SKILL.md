...
7. `progress.md` 中当前阶段 / 下一步技能是否为 `待计划` / `plan-change`，或是否仍缺少 plan/tasks。
...
- 如果 `plan.md` 与 `tasks.md` 已补齐且无阻塞，将 `progress.md` 更新为：`当前阶段：待实现`，`下一步技能：implement-change`。
- 在“进展记录”追加本次创建或更新的 plan/tasks 路径。
- 如果仍有阻塞，将当前阶段写为 `阻塞`，下一步技能写为“先处理阻塞项”，并记录阻塞原因。
