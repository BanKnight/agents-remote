...
- 保存项目认知 big picture：项目定位、用户场景、领域概念、项目结构、技术栈、架构边界、易错点、开发/验证准则与长期术语。
- 本文件渐进式补全，不要求一次完整。
- change 完成并通过 verify 后，应检查是否有长期项目认知需要提炼到这里；如果不更新，应在 distill 输出或 progress 记录中说明不更新理由。
- `docs/project.md` 应积极维护开发期 big picture，尤其是项目结构、技术栈、质量门禁、长驻进程/后台依赖管理、workflow 归档规则、验证 artifact 要求和反复容易犯错的边界。
- 不记录单次需求、任务状态或临时实现细节。
