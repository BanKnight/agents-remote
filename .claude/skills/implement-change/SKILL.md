...
- `progress.md` 中当前阶段 / 下一步技能为 `待实现` / `implement-change`。
...
7. `progress.md` 中当前阶段 / 下一步技能是否为 `待实现` / `implement-change`，或是否仍有未完成 tasks。
...
- 如果所有 tasks 已完成且无阻塞，将 `progress.md` 更新为：`当前阶段：待验证`，`下一步技能：verify-change`。
- 在“产物检查”中把 implementation 标记为已完成。
- 在“进展记录”追加本轮完成的任务摘要和 tasks 进度。
- 如果还有未完成 tasks，保持 `当前阶段：待实现`，`下一步技能：implement-change`，并记录剩余任务。
- 如果遇到阻塞，将当前阶段写为 `阻塞`，下一步技能写为“先处理阻塞项”，并记录阻塞原因。
