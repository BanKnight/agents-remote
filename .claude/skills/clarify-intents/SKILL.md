...
6. 在会话中暂存已经稳定的候选意图，并尽量拆成后续实现可引用的小粒度条目。
7. 通常每积累约 5 个稳定候选意图再批量写入或更新 `.workflow/intents.md`，不要每次回答都频繁写入。
8. 写入后继续追问下一个高价值分支，不因为单批意图已记录就结束。
...
记录到 `.workflow/intents.md` 的每条意图会作为后续实现、spec、design 或 roadmap 的引用单元，因此应尽量满足：
