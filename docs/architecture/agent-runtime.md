...
- Agent Runtime 负责生命周期、event stream、resume/reconnect 和 capability negotiation。
- Provider Adapter 吸收 Claude/Codex 差异，并将 provider-native identifiers、schema、auth/account state 和 compatibility 处理留在 adapter/internal metadata 中。
...
- `Session registry`：维护 internal session id、project、display name、provider、provider metadata 和 runtime state。
- `Agent runtime`：负责 AgentSession lifecycle、event stream、resume/reconnect、capability negotiation 和 provider adapter 调度。
