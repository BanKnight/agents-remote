...
| 模式 | 呈现 | 数据源 | 会话组织 | 运行时 |
|------|------|--------|---------|--------|
| Agent | 现有「按项目分段的单列网格」（§5），不变 | `useGlobalInstanceCandidates` / project sessions | 按项目分段（绑项目） | 现有 claude/codex runtime（不变） |
| Chat | 全局会话列表（不绑项目）+ 搜索 + 新建 | `useChatSessions`（`/api/chat-sessions`） | 全局（无项目概念） | pi SDK 库嵌入（Phase 3 后接入，Phase 1 占位） |

- **URL `?mode=agent|chat`**：mode 是一级 tab，URL 即真相，默认 `agent`（省略 = agent）。`validateWorkbenchSearch` 加 `mode` 字段白名单（照搬 `tab` 范式）。不加记忆 atom（URL 即真相）。**mode 与 rightTab/tab/leftMode 同属正交 search 维，全部经 `stickyWorkbenchSearch()` 统一粘性透传**（navigate 整体替换 search，漏带即丢）——所有 workbench navigate 调用点（中栏 tab focus、右栏切换、关 tab、进/出项目）只调这一个合并函数，新增 search 维度或新增 tab 类型的 focus 导航不再手抄 search 合并（mode 曾因各调用点手抄漏掉，点中栏 tab 后 chat 模式丢失、左栏语境被打掉）。
- **mode tab 位置（桌面/移动对齐）**：mode tab = 一级会话页顶部标题。桌面 = 左栏 `PanelHeader` title（`SessionModeTabs`，取代原 `t("nav.projects")` 文案标题）；移动 = `MobilePageHeader` title（`SessionModeTabs`）。同一 `SessionModeTabs` 组件（provider-agnostic，只调 `useWorkbenchNavigate` + `ModeTabGroup`），桌面/移动共用避免双写漂移。出现条件 `sessionPage = global scope + leftMode=auto`，**与 focusId 解耦**——中栏聚焦（session/chat/file/git/skill 任何 tab）是正交维度，不改左栏语境（VSCode 式，同 leftMode 粘性语义），mode tab 恒常驻、不掉回「会话」文案标题；其余左栏标题不变（项目 scope = 项目名返回头、files = 「文件」、plugins = 「插件」）。⚠️ 新增中栏 tab 类型时不要在 sessionPage 按 focusId 前缀加分支——左栏语境只由 scope/leftMode/mode 决定，tab 类型差异收敛在 focus effect 与 PanelRouter。chat 聚焦（focusId=`chat_*`）时 mode 由 `deriveWorkbenchRouteContext` 兜底（chat_* ⇒ mode=chat，数据不变式：chat 只在 chat 模式创建/打开），URL mode 键丢失也不掉语境。
...
- **tab = 实例**：每个 group 含 1-N tab（每个 tab = 一个 agent/terminal session），同 group 同时只渲染 active tab 的内容；其他 tab 用 CSS `hidden` 保留（**不 unmount**，保 WebSocket/relay 长连，避免 claude relay 重连丢早消息 + xterm dispose/重建抖动）。
...
| 自动重试开关（⟳） | tab 栏 ℹ 左侧 rotate 按钮（仅 claude agent session tab；enabled 常显高亮 `text-primary`，off 态 hover 显隐与 ℹ✕ 一致） | 点击即切即存（`useAutoRetryToggle`：读 detail 缓存 config 只切 enabled，无 config 预填默认文案；成功 invalidate detail，pending 置灰防连点）。参数编辑仍走 ℹ sheet 行内「开关+编辑」（2026-09-09 操作前置：高频开关不进 sheet） | 是 |
...
- **tab 右键菜单**（最小化 + 关闭实例）：右键 tab 栏 tab 弹出轻量菜单——「最小化」= 同 tab ✕（`removeTabFromGroup`，session 存活）；「关闭实例」= `useCloseSession`（自带 confirm → close API → 失效缓存）。kill 走此低频入口 + 左总览卡片 close，避免高频 tab ✕ 误触破坏性 kill。菜单视觉走 DESIGN.md `action-menu` 桌面 popover 变体（移动端不可达，刻意保留为桌面快捷）。**tab 栏 ℹ 按钮（2026-08-17，桌面实例信息入口）**：session tab chip 在 ✕ 左侧加 ℹ 按钮（`h-4 w-4` 与 ✕ 同尺寸，hover 显隐与 ✕ 一致），点击弹**居中 modal** 实例信息（`InfoSheetDialog` modal 形态，`useInstanceInfoSheet` variant 参数，移动端保持底部 sheet）。装配复用 `useInstanceInfoActions`（从 `mobile-workbench.tsx` 提取到 `instance-area.tsx` 共享，移动端 + 桌面端同一字段装配单一来源）。file/git/skill tab 无 session 生命周期，不渲染 ℹ。**tab 栏自动重试开关（2026-09-09，操作前置）**：claude agent session tab 在 ℹ 左侧加 rotate icon 开关按钮（`AutoRetryHeaderButton variant="tab"`，`h-4 w-4` 同尺寸）：enabled 常显 `text-primary` 高亮（用户主动开启的功能要一直可见，不受 hover 显隐影响），off 态与 ℹ✕ 同款 hover 显隐降噪；点击即切即存（`useAutoRetryToggle`），pending 置灰。参数（文案/延迟/窗口）仍走 ℹ modal 行内「开关 + 编辑」（`InfoField.action` 槽）——sheet fields 是打开时刻快照，行内操作面是自订阅组件（`AutoRetrySheetAction`），点击后实时反映新值。移动端聚焦态同款开关在 ℹ✕ 胶囊前置（`variant="capsule"`，`h-8 w-8`，off 态 `text-on-surface-soft` 常显不 hover 隐藏）。
...
### 7.7 移动端：drawer + 内容 tab 带（2026-08-16 重设计，2026-08-17 drawer 改 Reddit 式 push 刚体联动）
...
- **drawer Reddit 式 push 刚体联动（2026-08-17 用户决策：覆盖式 → 推开；同日三次反馈修正收敛）**：①「整个页面（含 header tab 带）作为一个整体被推」非只推内容区；②「被推页面保持视口宽不被压缩、保持大小仅仅被往右推」（flex 分栏把内容压缩到 50px 窄条重排是错的）；③「侧边栏没动画（静态避让）+ 恢复时两页叠加」——二次版 drawer absolute 静态、页面单独滑回时从 drawer 上方扫过产生叠加。最终结构 = **裁剪窗口 + 单一平移行**：drawer 与页面并排在同一 flex 行（永不重叠：drawer 行首 `shrink-0 basis-[min(88vw,340px)]`、页面 `w-full shrink-0` layout 宽 = 视口宽），行 wrapper 整体 `translate-x-0` ↔ `-translate-x-[min(88vw,340px)]`（`transition-transform duration-300 ease-in-out`）——打开 = 0（drawer 在窗口左 + 页面紧随其后并排），关闭 = -340（drawer 移出窗口左缘、页面正好填满窗口）。**刚体一起动**：drawer 从左滑入 + 页面同步被推右（同速同向），关闭一起回、无叠加；transform 不改 layout 宽度，页面内容零重排零压缩。**高度 = 旧覆盖式等价全高（2026-08-17 第四次反馈「高度缺一块」修正）**：push 初期把 safe-area 顶带避让统一收到 `main`（`pt-[var(--shell-safe-area-top)]`），整行被压到刘海带之下，drawer 顶上漏出 main 底色（`bg-surface/20` vs drawer `bg-surface`）且比旧全屏矮一块。修正 = 平移行**从 main 顶开始全高**（project 分支 main 不再吃 pt-safe-area），safe-area 顶带下放两侧各自消费：页面成员 `pt-[var(--shell-safe-area-top)]`（tab 带仍在刘海下，页面侧视觉零变化）+ drawer 壳 `pt-[var(--shell-safe-area-top)]`（`bg-surface`/圆角延伸进刘海带，内容避让）；drawer 顶行改 `h-11` 与页面 `MobileTabStrip` header 同构 → **两页顶行分隔线水平对齐**（探针断言 drawerH≈viewportH、drawerY=0、两行 bottom 相等）。无 scrim；关闭交互：点页面（父级透明拦截层 `absolute inset-0 z-10` 无视觉，关闭时不渲染不拦截）+ Esc（`useEffect` 手动监听 window keydown，原 Radix dismissable 职责）+ 点列表项/新建自动关（行为与覆盖式时期一致）。非 modal（无 focus trap / body scroll lock）——工作台 main 本身固定高 `overflow-hidden`，drawer 内部自滚，无泄漏，可接受。嵌套 Dialog（ActionMenu sheet / prompt / PagesRootDialog）从普通 aside 打开，反而消除 Dialog 套 Dialog。
...
- **header 内容 tab 带 ◄ 返回按钮（2026-09-07 用户决策）**：`MobileTabStrip` ☰ 右侧恒常渲染 ◄「返回项目列表」按钮（`project.backToProjects`，navigate 回 `/`）——聚焦 agent 详情后唯一退出路径原是「开 drawer → 点左上角返回」，路径过长；语义 = 离开项目回项目列表（与 drawer 左上角返回等价），浏览态/聚焦态恒常显示。
...
- **顶部行项目切换器（2026-08-17，`ProjectSwitcher` 组件，移动 drawer + 桌面左栏共用）**：drawer 顶部行项目名（返回按钮旁）与桌面左栏 header 项目名同为**可点击切换器**——点击弹项目列表（`OptionMenu`：移动底部 sheet / 桌面 popover，当前项目勾选 disabled，`cancelLabel` 传 `t("cancel")`），点其他项目 `navigate /projects/$key` 直接切换，免「返回 → 再进入」两步。数据源与全局项目列表同源（`useGlobalInstanceCandidates` kind=global → `["overview"]` cache，staleTime 5s 删建项目自动 invalidate）；overview 未就绪/失败或 projectNames 为空 → 降级纯项目名不可点。切项目 → `key={scope.key}` 重挂 → drawer 回浏览态默认展开。
- **新建会话入口移到 header 右上角**（2026-08-16 迭代）：二级页无底部 nav，FAB「落 nav 带、nav 收缩让位」语义锚点消失。`MobileTabStrip` trailing 由「仅聚焦 session ℹ✕ 胶囊」扩展为二态——浏览态（无 focusId）= icon 方新建按钮（与 ☰ 对称，`h-9 w-9`，ActionMenu Claude/Terminal 底部 sheet）；聚焦 session = ℹ✕ 胶囊；file/git/skill 聚焦 = 空。移除 `MobileProjectWorkbench` 的 `<MobileFab>`。**同日后续迭代：全部 FAB 下线**——新建项目（`MobileGlobalOverview` `MobilePageHeader.actions`，dialog 逻辑收敛 `useCreateProjectDialog()` hook）、新建文件夹/上传（`FilesPanel` header 行右侧 `max-lg:flex` ActionMenu）、新建页面根（`PagesPanel` `lg:hidden` header 行）全迁 header 右上角；`MobileFab` 组件删除、底部 nav `group-has-[.mobile-fab]` 让位 CSS 清理。**2026-08-17 用户修正「右上角 = 更顶部的 header 右上角」**：项目工作台新建入口从段内 header 行**提升到 drawer 顶部行右上角**（返回+项目名那行，按当前段切换动作：总览=新建 Claude/终端、文件=新建文件夹/上传、页面=新建页面根；历史/Git/Wiki/插件段不显示）；FilesPanel / PagesPanel 不再内嵌移动入口（聚焦态 tab 内容是查看语义）；新建页面根经 `PagesPanel` `createRequest` 信号 prop 触发（详见 DESIGN.md `floating-action-button` 条目与 frontend-notes §12）。
- **聚焦态 header ℹ✕ 操作（2026-08-17，探针 `probe-mobile-focus-actions.mjs`）**：聚焦态 header trailing `MobileFocusActions`（ℹ 实例信息 / ✕ 关闭实例）修复两个「点击无响应」——根因都是 **holder 状态更新了但没渲染进组件树**：① ℹ 调 `useInstanceInfoSheet().open` setPending，但组件 JSX 漏渲染 `{infoSheet.holder}`（桌面 `MobileFocusHeader` 有，移动端漏）→ sheet 永不挂载；② ✕ 走 `closeInstance` → `useCloseSession` confirm setPending，但 `closeHolder`/`renameHolder`/`createPromptHolder` 原本只在 `MobileProjectWorkbench` **浏览态分支**渲染，聚焦态分支没有 → confirm 永不挂载。修复 = ① `MobileFocusActions` 补 `{infoSheet.holder}`；② 三个 holder 从浏览态分支**提升到组件顶层**（`<>...</>` Fragment 内、裁剪窗口外，浏览/聚焦都挂载）。**通用教训：holder（useConfirm/usePromptDialog/useInstanceInfoSheet 的 `{holder}`）必须渲染在组件树稳定位置，不能放进随浏览↔聚焦切换的条件分支**——否则触发 holder 的操作在切换后的分支里「点击无响应」。
- **tab 带 3 个行为修正（2026-08-17，探针 `probe-mobile-tab-strip.mjs`）**：① 新建 tab 应成为激活 tab——drawer 总览段新建（`useCreateSession` onSuccess navigate）后 `focusId` 变化 → `MobileProjectWorkbench` effect 自动关 drawer（`useEffect([focusId])`，覆盖 focusInstance 手关之外的路径），新激活 tab 立即可见；② 激活 tab 滚入视野——`MobileTabStrip` auto-scroll：activeTabId 变化时算 `scrollLeft` 让 active chip 完全可见（只动横向容器，不用 `scrollIntoView` 避免连带页面滚动）。**实现要点**：chip 宽度依赖 `usePanelMeta` 异步查询（meta 到达前 fallback 短 label + 无 marker，chip 窄；meta 到达后变宽），仅一次 deps 触发的对齐会停在旧布局算出的目标——故首次对齐后加 `setInterval` 200ms 周期校准直到「不溢出」（布局稳定自停）；③ **切 tab 不重连 = 「全保活 + 聚焦过即可」**（用户决策：与桌面端方案必然不同，移动端单面板一次只显示一个 tab）。聚焦态渲染「本会话聚焦过的已打开 tab（含当前激活）」的多个面板，非激活用 `hidden` class 保持挂载（WS 不断），面板 `key={tabId}` 稳定实例（非旧 `key={focusId}` 每切 tab 卸载重挂）。`focusedTabIds` Set（`useState` 初始仅当前激活 + effect 随 focusId 变化逐步纳入）实现「聚焦过即可」：刷新/进入时 layout 恢复的未聚焦 tab **不预挂载**（只挂当前激活），随切换激活逐步纳入保活；浏览态（无 focusId）保活面板保持 hidden 挂载——回聚焦态零重连；tab ✕ 时从集合移除。
