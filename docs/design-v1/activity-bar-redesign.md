...
45. **设置弹窗细节修复（批 R）**：批 Q 后实测 6+1 项。① **加载态固定结构**——`SettingsContent` 加载中即时渲染两段框架（ProvidersSection skeleton 行 + ClaudeRuntimeSection `EMPTY_RUNTIME` + 控件 disabled），加载完 remount 回填，不用整页空白/整页骨架。② **ProviderDialog 字段顺序** label → protocol → baseUrl → apiKey；③ **provider 文案** `settings.label`「标签/Label」→「名称/Name」；④ **apiKey 明文**（去 `type=password` + `autoComplete=off`，个人部署不触发浏览器保存密码）；⑤ **providers 列表 `max-h-72 overflow-y-auto`** 独立限高，runtime 段不被挤；⑥ **OptionMenu 嵌套**——桌面 `DropdownMenu modal={false}`，修 SettingsDialog modal 内 runtime provider/effort/model tier 点击无反应（Radix 双 modal pointer-events 冲突）；⑦ **providers Apple grouped 列表**——`Card gap-0 py-0` + `ListGroup`/`ListRow` divide-y，**整行点击进 ProviderDialog 编辑详情**，删除收进行尾 ⋯（`stopPropagation` 防冒泡编辑）。浮窗卡片视觉（`rounded-2xl shadow-2xl` + header ✕）批 Q 已就位、本批探针验证。
46. **设置 runtime 段加 Codex 页签（结构预留，UI-only）**：runtime 段从「Claude 专属」升级为「多 runtime 切换器」——`ClaudeRuntimeSection` 拆为 `RuntimeSection`（section 外壳 + `SegmentedControl[Claude|Codex]` 本地 tab state，默认 claude）+ `ClaudeRuntimeContent`（原 Claude `<Card>` 主体 + 全部 state/save 逻辑，`key={loading?"loading":JSON.stringify(runtime)}` remount 跟着它走）+ `CodexRuntimeContent`（**诚实占位**：「Codex 运行时即将支持」，**无伪造控件**）。Codex agent 尚未支持，**纯 UI 不碰后端**——不动 shared `SettingsState` / `runtimes.codex` / settings-store，Codex tab 是本地 `useState` 不持久化；结构现在就位，未来 Codex 接入 = 填 `CodexRuntimeContent` + 接 backend，非重构弹窗。`settings.runtime` section label 「Claude runtime」→「运行时」（现在是 multi-runtime）；`settings.runtimeHint` 同步泛化。**抽 `SegmentedControl<T>` primitive**（`shell-primitives.tsx`，Apple 风格 inline 分段：`role=group`+`aria-pressed`+`min-h-11` 触摸友好，active `bg-primary/15 text-primary`），**收口** ProviderDialog 协议选择器（原 inline 同款范式 → 用 primitive，零视觉变化）；调用点 `settings-dialog.tsx` protocol + runtime tab。

47. **provider 协议契约落地（批 S，UI-only）**：批 R/Codex tab 把双协议结构铺好后，provider 与 runtime 之间的协议契约补齐四处。① **ProviderRow 协议彩色 chip**——subtitle 从单行 `apiKeyMasked · baseUrl · protocolLabel` join 改富文本三段：协议 chip（`PROTOCOL_CHIP_CLASS`：anthropic `bg-primary/15 text-primary`、openai-compatible `bg-on-surface/10 text-on-surface-soft`，`text-[10px] font-semibold rounded-md`）+ baseUrl 主文本（`text-on-surface` font-mono）+ apiKeyMasked 次要（`text-on-surface-muted`）；`flex-wrap` 窄屏换行不溢出。扫读即知「哪几个 provider 能给某 runtime 用」。② **baseUrl 必填（UI 校验）**——`handleSubmit` 加 `baseUrl.trim()` 非空校验（`settings.baseUrlRequired`），create 去掉 `|| undefined` 始终传；**不收紧** shared `ProviderConfig.baseUrl?`（向后兼容历史 providers.json），与批 Q/R UI-only 取舍一致。③ **runtime 按协议过滤**——新增 `RUNTIME_PROVIDER_PROTOCOL = { claude:"anthropic", codex:"openai-compatible" } as const satisfies Record<"claude"|"codex", ProviderProtocol>` 契约常量；`ClaudeRuntimeContent` OptionMenu items 改用 `providers.filter(p => (p.protocol??"anthropic") === RUNTIME_PROVIDER_PROTOCOL.claude)` 派生的 `runtimeProviders`（Claude 只见 anthropic provider）；`selectedLabel`/`selectedProtocol` 仍用全量 providers 查（stale 选择——选中后被改成不兼容协议——trigger 仍显名、可改选 None/兼容项）；Codex 侧占位未接 provider 但契约常量已声明，后端接入时直接复用。④ **文案通用化**——`providersHint`「Claude API 凭证…」→「管理 API provider 凭证与端点。Claude runtime 用 Anthropic 协议 provider；Codex runtime（即将支持）将用 OpenAI 兼容协议 provider」（呼应③的过滤契约）；`baseUrlHint` 去「可选/Anthropic」改必填通用描述；placeholder `https://api.anthropic.com`→`https://api.example.com`。新增 DESIGN.md `protocol chip` 条目。
