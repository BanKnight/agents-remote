...
右栏是 **tab 式**（文件/Git/插件…），一次只显示一个 tab；正因是 tab 结构，才能作为「插件预留位」——未来新功能以新 tab 形式挂入。
...
- **移动**：两层路由
  ...
  - 二级：`/projects/:key`（默认总览）/ `/projects/:key/files|git`
  - 实例：`/projects/:key/session/:id`（单实例聚焦，含输出/文件/git tab）；全局进 = `/global/session/:id`
...
| 项目内实例区 + 右栏 inspection | 二级 header tab（总览/文件/git） |
| split 并排多实例 | 单实例聚焦 + ‹ › 顺序切 |
| 右栏 inspection tab | 单实例 header tab 的文件/git（项目级同内容） |
