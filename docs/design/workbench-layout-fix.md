...
- **e2e 基线债已清**（`bun run e2e` 20/20 绿，1.9m）。诊断后归 4 层，**混合来源——非全 pre-existing**：
  1. **项目节点找不到**（claude×3 / pwa / terminal，原 15 失败的主因）：global scope 默认 grid view 只显实例，e2e `demo` 项目无实例 → 空态无项目节点按钮。**fix** = `workbenchViewAtom` 默认 `grid`→`grouped`（`workbench-model.ts`；global scope 是项目总览，应显含空项目；project scope 因 `filterWorkbenchViews` 隐藏 grouped → atom=grouped 时 resolvedView 回退 grid，不受影响）。
