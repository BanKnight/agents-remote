...
- **下一步**：核心 4 问题 + e2e 基线债全修（20/20 绿），本轮布局修复收尾。
...
- **e2e 基线债已清**（`bun run e2e` 20/20 绿，1.9m）。诊断后归 4 层，**混合来源——非全 pre-existing**：
  1. **项目节点找不到**（claude2×3 / pwa / terminal，原 15 失败的主因）：global scope 默认 grid view 只显实例，e2e `demo` 项目无实例 → 空态无项目节点按钮。**fix** = `workbenchViewAtom` 默认 `grid`→`grouped`（`workbench-model.ts`；global scope 是项目总览，应显含空项目；project scope 因 `filterWorkbenchViews` 隐藏 grouped → atom=grouped 时 resolvedView 回退 grid，不受影响）。
  2. **Layer A 中栏 nav 回归**（middle-tab-left ×5 + file-nav）：**本轮 phase 1**（commit `eb5d8e1` 移除 GlobalNavNode）误删了 `ProjectLeftPanel` middle tab bar 的 `<nav aria-label=Projects>` 包裹（5d9178a 时存在，spec `projectsNav=getByRole("navigation",name="Projects")` 依赖它），phase 1 连带 GlobalNavNode 的 nav 一起删了。**fix** = 重新加回 nav landmark（`project-left-panel.tsx`，组件修复还原 a11y 地标，非 spec workaround）。
  3. **Layer B 重复 aria-label**（file-browser + git-diff）：中栏 keep-alive 多 tab——inactive tab panel 用 `display:none` 不 unmount 保 WebSocket/relay 长连（§7.4，`instance-area.tsx:2000` panels.map），故多 file/git tab 各带一个 `<section aria-label="File preview"/"Git file diff">` 在 DOM；`getByLabel` 按 DOM 匹配不过滤 hidden → 命中多个 strict mode。**git 版本由本轮 phase 3**（commit `2794c98` 新增中栏 `GitFileDiffPanel`）引入，file 版本 keep-alive 既有。组件 a11y 本就正确（display:none 已从 a11y tree 移除，屏幕阅读器只读活动 tab），**fix** = spec `getByLabel`→`getByRole("region",...)`（visibility-aware，默认排除 hidden，只命中活动 tab 可见 panel）。
  4. **Layer C mobile Create 按钮**（mobile-nav）：Create 按钮在 `GlobalProjectsOverview` body ViewSwitcher 行（批 D 刻意位置），spec 错误限定到 `header`（`MobilePageHeader` 仅标题无 actions）。**fix** = spec Create 选择器从 `header` 改 `page`。
  - memory `feedback-baseline-harness-must-be-green` 达成。教训：本轮 phase 1/3 引入了 2 层回归（nav 删除 + git diff 重复 aria-label），phase 提交时未跑完整 e2e；后续每 phase commit 前应跑 `bun run e2e` 而非仅靠单测门禁。
