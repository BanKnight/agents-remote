...
**`minimumContrastRatio`（256 色 / truecolor 可读性兜底）**：上述 `--terminal-*` 仅映射 ANSI 16 色（`\e[3xm`/`\e[9xm`）；程序（如 claude CLI）常用 **256 色**（`\e[38;5;N m`）或 truecolor 输出色，走 xterm 内置固定调色板，**不受 theme token 控制**。实测 claude CLI 用 `38;5;153`（浅蓝 `#87D7FF`，选中高亮）、`38;5;220`（金，标题）、`38;5;246`（灰，次要文字）——这些浅色在浅底上对比度极低（字芯淡、抗锯齿边缘融入浅底显「白色描边」/发糊），暗底上则高对比清晰。故 `XtermOutput` 设 `minimumContrastRatio`：**亮色 `4.5`（WCAG AA，启用）/ 暗色 `1`（xterm 默认，关闭，零变化）**——xterm 自动给低于阈值的彩色加深前景至达标（claude 浅蓝 → 可读深蓝），暗色完全不启用。效果是**全局**的（亮色下所有低对比彩色都会提对比，不只 153），随 `resolved` 切换；仅实时流可见的渲染效果交用户真机确认（自动化只能验 options 被正确设置，验不了像素）。

**`extendedAnsi`（256 色源头精准映射，续七）**：`minimumContrastRatio` 是事后算法加深，颜色由 xterm 决定不可控。xterm.js 公开 API `ITheme.extendedAnsi?: string[]`（typings `ANSI extended colors (16-255)`）可**从源头替换 256 色调色板 16-255 档**——`ThemeService._setTheme` 按下标 `i` 写入 `colors.ansi[i+16]`，`parseColor("")` 抛错走 fallback 保留 `DEFAULT_ANSI_COLORS[i+16]` 默认（**空串 = 保留默认**，契合稀疏覆盖）。Solarized 官方只有 16 个精确色值、无现成 256 色 cube，故不做全量 240 档 CIELAB nearest-color（避免 htop/vim 渐变退化），改为**精准映射 claude 实测的 3 个索引**到 Solarized Light 精确色：`153` → blue `#268bd2`（选中高亮）、`220` → yellow `#b58900`（金标题）、`246` → base01 `#586e75`（灰次要）。仅亮色挂（数组长度 231 = 覆盖到 246，其余空串保留默认）；暗色 `extendedAnsi: undefined` → 全用 xterm 默认，零变化。与 `minimumContrastRatio` 分工：`extendedAnsi` 源头替换 256 色（精确可控），`minimumContrastRatio` 兜底 truecolor（`38;2;r;g;b` 绕过调色板，extendedAnsi 管不到），两者并存。
