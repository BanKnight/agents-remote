...
- **移动工作台**：两层导航——一级底部胶囊（项目/全局/设置）+ 二级单行 header。**一级底部胶囊 `mx-auto` 始终水平居中（中心恒定，不随 FAB 出现/消失平移）；宽度随有无 FAB 变化——无 FAB 页按内容收缩（`w-fit max-w-full`）撑满舒展，item `px-4` + `gap-4`；有 FAB 页 `group-has-[.mobile-fab]:max-w-[calc(100vw-8.75rem)]`（连续自适应，8.75rem=140px=2×(FAB 占位 68px+间隙 2px)）收窄让位（两侧对称留白，FAB 落右侧留白，**紧贴 FAB 间隙 2px、中心恒居中、各视口自适应**：390w→250/375w→235/360w→220）+ `group-has-[.mobile-fab]:gap-1` + item `group-has-[.mobile-fab]:px-1` 收紧（参考苹果 tab bar 居中 + 自身收窄让位范式，非旧 `pr-[4.5rem]` 左推）**。列表态与聚焦态 header 同款单行结构：◄ 返回 + tab 横滚区（`flex-1 overflow-x-auto` 隐藏滚动条）+ 右侧 `shrink-0` 区（列表态=项目名/全局标题，聚焦态=ℹ✕ 胶囊操作区）。**移动 header 高度统一 `h-11`、padding `px-3`**：`MobilePageHeader`（一级大标题式：项目列表 / 设置）与 `MobileTabHeader`（tab 横滚式：全局总览 / 项目总览 / 聚焦态）两套 primitive 并存但视觉高度 + padding 对齐，覆盖所有移动 header。项目列表态合并旧「MobilePageHeader + 二级 tab 行」为单行（对齐聚焦态），tab 多时横滚不换行，项目名右侧 `truncate`。聚焦态 = 单实例内容，底部让位给输入区。三段式 grid：`header` 置顶 / `content minmax(0,1fr)` / `input 或 nav` 置底，超出只在 content 滚动。
