...
- **行高 = padding + max(marker, 文本行盒)**。例：`ListRow` 行高 = `py-2.5`(20px) + marker(`IconMarker sm` 28px) ≈ 50px；`ListRowSkeleton` 必须用同样 `rounded-xl px-3 py-2.5` + `h-7` marker，实测骨架行高 = 真实行高 = 50px（delta 0）。
...
- **不模拟高方差字段**：Git status 标签 / File size 各异，占位反失真——`ListRowSkeleton` 只占 marker + title，不模拟 meta。
...
| `ListRowSkeleton`（shell-primitives） | `ListRow`（`rounded-xl px-3 py-2.5` + marker `h-7`） | Git 文件列表、File 列表 |
