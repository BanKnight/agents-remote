...
- **mobile-sheet-fullscreen**：移动端全屏覆盖 sheet（file preview 等 contextual deep view）。`fixed inset-0 z-50` + `bg-surface`（不透明，遮挡父页面）+ `pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]` 单点避让（不叠 vh/dvh，详见 `frontend-notes.md` §1）+ `animate-in slide-in-from-bottom-full duration-300 ease-out`（`tw-animate-css`）。z-50 高于底部一级 nav。关闭按钮放 header 右上，`text-primary`（iOS Done 风格的 button-secondary 强调版）+ `sm:hidden`。桌面 `sm:static sm:flex-1 sm:bg-transparent sm:pt-0 sm:pb-0 sm:animate-none` 回 inline。
