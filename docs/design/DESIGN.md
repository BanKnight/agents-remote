...
| `GroupedProjectsSkeleton`（instance-area） | `GroupedProjectsList`（每组 section：项目名行 `flex items-center gap-2 px-2 py-1.5` = 图标占位 `size-4` + 项目名条 `h-5 w-1/3` 对齐 `text-sm font-semibold` 行盒 20px + ⋯ 占位 `size-9` 对齐 ActionMenu trigger；实例区小标题 `flex items-center gap-1.5 px-3 py-1` = chevron `size-3` + 文本条 `h-4 w-16` 对齐 `text-xs` 行盒；`CardGridSkeleton plain` 每组 2 卡） | grouped 视图加载 |
