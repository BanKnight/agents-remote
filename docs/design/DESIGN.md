...
- **carousel（paged-card，批 J / 决策 33）**：实例分组横向分页（每页 N 卡）。容器 `overflow-x-auto snap-x snap-mandatory snap-center` + 隐藏滚动条（`[scrollbar-width:none] [&::-webkit-scrollbar]:hidden`）；每页 `shrink-0 w-[calc(100%-3rem)] snap-center`（`-3rem` = 左右各 24px peek，snap-center 居中使双侧露出邻组边缘作翻页提示），页内卡片纵向复用 `InstanceGrid` 单列 `plain`。当前页同步靠 `onScroll` 算页码。**移动端**靠原生左右 swipe 翻页；**桌面端**（不可触摸）配页码行 `hidden lg:flex`（`‹` prev + 页码 button `aria-current` + `›` next，onClick 目标页 `scrollIntoView({behavior:smooth,inline:center})`）。**≤1 页退化**：直接 `InstanceGrid` 渲染，无 peek 无页码行（实例数 ≤ pageSize 时无分页语义）。peek 量 24px×2 满足「明显看出漏出邻组」。
...
| `GroupedProjectsSkeleton`（instance-area） | `GroupedProjectsList` 批 J（每组 section：项目名行 `flex items-center gap-2 px-2 py-1.5` = 图标占位 `size-4` + 项目名条 `h-6 w-1/3` 对齐 `text-base font-semibold` 行盒 24px + › 占位 `size-5` + ⋯ 占位 `size-9` 对齐 ActionMenu trigger；**无实例区小标题**（折叠废弃，决策 33）；`CardGridSkeleton plain` 每组 3 卡 = carousel 一页 peek 结构） | grouped 视图加载 |
