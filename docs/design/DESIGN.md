...
| `GroupedProjectsSkeleton`（instance-area） | `GroupedProjectsList` 批 J + 批 L（每组 section `overflow-hidden rounded-lg border border-neutral-line bg-surface-raised`（两端统一带 bg 边框，2026-08-03 撤销决策 38 移动无边框）；根 `px-3 py-3` 两端统一）：项目名行 `flex items-center gap-2 px-2`（批 L 去 py）= 图标占位 `size-5`（批 K）+ 项目名条 `h-6 w-1/3` 对齐 `text-base font-semibold` 行盒 24px + › 占位 `size-5` + ⋯ 占位 `size-9` 对齐 ActionMenu trigger；**无实例区小标题**（折叠废弃，决策 33）；实例区 `-mt-2` 包 `CardGridSkeleton plain` 每组 3 卡 = carousel 一页；`CardGridSkeleton` plain 去 `divide-y`、占位卡 `left-15`（两端 inset）分割线 mirror 真实） | grouped 视图加载 |
