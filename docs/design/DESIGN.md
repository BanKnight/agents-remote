...
- **navigation-drawer**（移动端项目侧边栏，`mobile-project-drawer.tsx`）：**push 并排抽屉**（2026-08-17 从 Radix `Dialog` 覆盖式改造——用户决策：侧边栏从左往右推、和原页面并排，内容被推到右侧**可见**而非被 scrim 盖住）。drawer 是 `MobileProjectWorkbench` flex 布局的固定部分（`<aside>` 非 portal、无 scrim、无 focus trap）；外层宽度过渡容器管 `w-[min(88vw,340px)]` ↔ `w-0` 切换（`transition-[width] duration-300 ease-in-out`，内容区 flex-1 被平滑推开/收回），本 aside **内层固定宽** + `overflow-hidden`（过渡期间内容不换行跳动，视觉是抽屉从右缘向内收起）。**关闭交互**（原 Radix dismissable 职责的替代）：点内容区（父级透明拦截层 `absolute inset-0 z-10`，无视觉、关闭时不渲染）+ Esc（组件内 `useEffect` 手动监听 window keydown）。**尺寸**：宽度 `min(88vw,340px)`（Material navigation-drawer：小屏 ≥88% 宽、上限 340px），右缘 `rounded-r-2xl`（16px，Shape 档 `2xl`）。**行模型**：段导航横向 tab 行（复用 `TabButton` + `overflow-x-auto` 可横滚，active `bg-primary/10 text-primary` nav-item token，对齐桌面左栏 middle tab bar）。**结构**：顶部返回入口 A（离开项目回列表）+ 项目名 + 右上角按段新建入口；段导航（7 段横向 tab）+ 段主体（总览=会话列表+新建入口/历史=HistoryList/文件=FilesLeftPanel 文件树·Git=GitChangesList 变更列表（点文件关 drawer 开 tab / diff tab，文件·Git 并入 tab 带）/页面·Wiki=PagesPanel/WikiPanel/插件=PluginsPanel）。**收起 = w-0 非 unmount**：drawer DOM 常驻（宽度 0 + overflow-hidden 隐藏），重新展开无重挂。**safe-area**：drawer 在 header 下方，顶部避让由 main 统一 `pt-[var(--shell-safe-area-top)]` 承担（本组件顶部行不叠）；底部 `pb-[env(safe-area-inset-bottom)]` 单点避让（frontend-notes §1）。
