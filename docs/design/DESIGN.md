...
- **移动触摸目标 44px**（apple-design §10 / Apple HIG）：移动端（<640px）会触摸的**行内/列表**操作按钮（如 table RowActions ⋯）须 `max-sm:h-11 max-sm:w-11`（44×44px 达 HIG）。**header 右侧操作区例外**：浮层 detail header close（file preview / git diff 的 `sm:hidden` close）、mobile-workbench focus header ℹ✕ 胶囊、总览 ViewSwitcher 等保持视觉紧凑尺寸（h-7/h-8），**不**放大到 44px——header 内按钮与相邻标题/tab 的视觉比例协调优先于触摸目标达标，容器整体放大后会与相邻元素不协调（Apple 标准范式应是「小视觉 + 大触摸区域」：如需达标用透明 padding 扩展热区，而非把容器本身撑大）。桌面鼠标专用按钮（`sm:opacity-0 sm:group-hover` 显隐、`lg:flex` 桌面工具栏）不限尺寸，保持紧凑。
