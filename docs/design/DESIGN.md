...
- **移动工作台**：两层导航——一级底部胶囊（项目/全局/设置）+ 二级单行 header。**一级底部胶囊 `mx-auto` 始终水平居中（中心恒定，不随 FAB 出现/消失平移）；宽度随有无 FAB 变化——无 FAB 页按内容收缩（`w-fit max-w-full`，不撑满、留两侧留白舒展），item `px-4` + `gap-4`；有 FAB 页 `group-has-[.mobile-fab]:w-full group-has-[.mobile-fab]:max-w-[calc(100vw-9.5rem)]`（9.5rem=152px=2×(FAB 占位 68px+间隙 8px)，`w-full` 让 capsule 真撑到 max-width 上限而非被内容拽小——`w-fit` 下 max-width 是够不着的天花板，中文短标签 capsule 只到 ~157px 间隙 ~48px）收窄让位（两侧对称留白，FAB 落右侧留白，**间隙 8px 不贴死、中心恒居中、各视口自适应**：390w→238/375w→223/360w→208）+ `group-has-[.mobile-fab]:gap-1` + item `group-has-[.mobile-fab]:px-1` 收紧（参考苹果 tab bar 居中 + 自身收窄让位范式，非旧 `pr-[4.5rem]` 左推）**。列表态与聚焦态 header 同款单行结构：◄ 返回 + tab 横滚区（`flex-1 overflow-x-auto` 隐藏滚动条）+ 右侧 `shrink-0` 区（列表态=项目名/全局标题，聚焦态=ℹ✕ 胶囊操作区）。**移动 header 高度统一 `h-11`、padding `px-3`**：`MobilePageHeader`（一级大标题式：项目列表 / 设置）与 `MobileTabHeader`（tab 横滚式：全局总览 / 项目总览 / 聚焦态）两套 primitive 并存但视觉高度 + padding 对齐，覆盖所有移动 header。项目列表态合并旧「MobilePageHeader + 二级 tab 行」为单行（对齐聚焦态），tab 多时横滚不换行，项目名右侧 `truncate`。聚焦态 = 单实例内容，底部让位给输入区。三段式 grid：`header` 置顶 / `content minmax(0,1fr)` / `input 或 nav` 置底，超出只在 content 滚动。
...
- **carousel（paged-card，批 J / 决策 33 + 批 M / 决策 36 + 批 N / 决策 37 + 批 P / 决策 39）**：实例分组横向分页（每页 N 卡）。容器 `overflow-x-auto snap-x snap-mandatory scroll-px-0` + 隐藏滚动条（`[scrollbar-width:none] [&::-webkit-scrollbar]:hidden`）；每页 `shrink-0 w-full snap-start`（满宽，无 peek/spacer），页内卡片纵向复用 `InstanceGrid` 单列 `plain`。当前页同步靠 `onScroll` 算页码（slot=scrollWidth/pageCount；满宽页等宽 = container，slot=container，snap 仍工作）。**两端卡片满宽**（2026-08-03，撤销决策 38-43 full-bleed peek 范式）：移动端不再靠双侧 peek 缩进 + swipe 暗示，改满宽卡片 + **dots 指示器**（`lg:hidden`，active `bg-primary` / inactive `bg-on-surface-muted/40`，复用 `carousel.pageLabel` i18n，点击 `goTo(i)`），= 桌面页码行 ‹1·2·3› 的移动等价 ●○○（平台差异合理表达：同一「当前页指示」语义的平台适配）；桌面端 `lg:w-full` 满宽 + 页码行 `hidden lg:flex`（`‹` prev + 页码 button `aria-current` 高亮 + `›` next，onClick 目标页 `scrollIntoView({behavior:smooth,inline:start})`，`inline:start` 对齐 `snap-start`）。**≤1 页退化**：直接 `InstanceGrid` 满宽渲染，无 carousel 容器、无 snap、无 dots/页码行（实例数 ≤ pageSize 时无分页语义）。**根 px-3 两端统一**（2026-08-03，撤销决策 42 移动 `px-0` full-bleed 贴边）：`GroupedProjectsList` 根两端统一 `px-3 py-3`，section 离屏幕边缘 12px 让 border+bg 可见；`GroupedProjectsSkeleton` 同步。**两端统一带 bg 边框**（2026-08-03，撤销决策 38 移动 full-bleed 无边框）：section 两端统一 `rounded-lg border border-neutral-line bg-surface-raised`（移动去无边框、桌面补 bg）——`bg-surface-raised` 比 shell(`surface`) 浮起一层，InstanceCard plain 透明露出正常卡片底色，复刻设置 Card grouped 带 bg 范式（设置 Card 用 `bg-surface` 因其底是 `bg-surface-raised/15`，本处分组父底是 shell 故用 `surface-raised` 达同款浮起关系）；InstanceCard plain 仅桌面 hover（`lg:hover:bg-on-surface/5`，移动无高亮）；分割线两端统一 inset（InstanceCard `topSeparator` `left-15`=60px 跳过 marker 列）。**名行 padding 两端统一**（2026-08-03，撤销决策 39-43 peek 配套）：`GroupedProjectsList` 名行 `pl-5 pr-7 lg:pl-2 lg:pr-2` → 两端统一 `pl-3 pr-2` + 进项目 button `px-0`（去 `lg:px-1`）——卡片满宽后名行不需 peek 缩进配套：图标 = pl-3(12) + button px-0 = 12 ≡ marker（InstanceCard p-3=12），⋯ = pr-2(8) ≡ 满宽 action（absolute right-2=8），两端图标≡marker、⋯≡action 严格对齐；桌面无回归（原 lg:pl-2(8)+lg:px-1(4)=12 ≡ 现 pl-3(12)+px-0=12）。⋯ 删除与卡片 ⋯ action 同尺寸同图标同列（均 `h-7 w-7 touch:h-10 touch:w-10` + `ShellIcon ellipsis h-4 w-4` 同源，维度正交化见「触屏触摸目标 44px」条目）。
...
- **floating-action-button (FAB)**（移动端主创造动作悬浮胶囊，iOS 范式）：移动端专属（`lg:hidden`，桌面端不渲染——桌面保留各自 header 入口：CreateSessionBar tab bar / 「+ 新建项目」header 按钮 / Files header actions，按视口分流而非 `useIsMobile`，与底部 nav 同款 CSS 阈值避免 639–1023px 间隙 + hydration 闪烁）。**定位 `position: fixed right-3 bottom-[var(--shell-safe-area-bottom,0px)] z-30`**——`fixed` 锚视口右下角（三外壳结构不统一，`absolute` 会锚到漂移的 positioned 祖先；三外壳/FilesPanel 路径无 transform 祖先，fixed 安全；若未来进 transform 容器改 `createPortal(…, document.body)`，先例 `Claude2SessionDetailRoute.tsx:1406`）。`bottom = safe-area`，与底部 nav 胶囊条**同基线**，落在内容滚动区已避让的 nav 高度带内（`pb-[--shell-mobile-bottom-nav-space]` 已为整条 nav 让位，`useMeasuredBottomNav` ResizeObserver 注入）——**FAB 不再凸入内容滚动区、零遮挡**，复用现有「滚到尽头」避让逻辑（苹果范式：FAB 与一级导航同处高度，一级导航收缩让位）。水平让位由 **nav 胶囊条自身收窄承担**——`group-has-[.mobile-fab]:w-full group-has-[.mobile-fab]:max-w-[calc(100vw-9.5rem)]`（9.5rem=152px=2×(FAB 占位 68px+间隙 8px)；`w-full` 让宽度真撑到 max-width 上限——`w-fit` 下 max-width 是够不着的天花板，中文短标签（项目/文件/技能/设置）capsule 只到 ~157px 间隙 ~48px；capsule 宽=视口−两侧对称让位，**间隙 8px 不贴死、中心恒居中、各视口自适应**：390w→238/375w→223/360w→208）+ `mx-auto` 居中，capsule 中心恒定不左推（撤销旧 `pr-[4.5rem]` 左推：FAB 出现/消失时 capsule 整体平移、位置突兀）；FAB 落 capsule 右侧对称留白。无 FAB 页 capsule `w-fit max-w-full` 按内容收缩不撑满（`main.group :has(.mobile-fab)` 控制，无 FAB 零副作用）。`z-30` > 底部 nav `z-20`。**底座** `actionButtonClasses({ tone: "accent", className: "size-14 rounded-full justify-center p-0" })`——size-14（56px 触摸目标，超 HIG 44px）+ rounded-full 圆形纯图标 `<ShellIcon name="plus" className="size-6" />`，accent token 自带 `from-primary to-secondary` 渐变 + `shadow-lg shadow-primary/25` + `active:bg-on-surface/10` press-feedback，不散写裸色。**两种模式**（单一 primitive `MobileFab`，`shell/mobile-fab.tsx`）：① 菜单模式（`items` 非空）= 底座 button 作 `<ActionMenu trigger>`，移动端点开弹底部 sheet、桌面 popover（ActionMenu 既有能力，复用 `action-menu` token）——项目工作区（Claude/Terminal，`create.createAgent/createTerminal`）、文件页 Files tab（新建文件夹→`usePromptDialog` 弹框输入名 / 上传→程序化 `fileInputRef.click()`）；② 直跳模式（无 `items`）= 底座 button 直接 `onClick`——全局总览（直开 ProjectSetupPanel Dialog）。**承载页面单一主创造动作**：解放拥挤的二级 header 行到拇指热区，header 回归纯导航/视图切换（移除移动专用 CreateSessionBar，桌面 CreateSessionBar 在 `InstanceLeftOverview` tab bar 不动）。**聚焦态不渲染**（FAB 挂在非聚焦态内容组件内，session/file focus 走 `MobileSessionFocus`/`MobileFileFocus` 不渲染宿主，FAB 随之不在）。**边界**：`/files` 全局文件页不引入（无写操作语义，`enablePreview=false`）；FAB 不承载「多主操作」（一页一 FAB，主操作唯一）。
...
| `CardGridSkeleton`（instance-area） | `InstanceCard`（marker + title/subtitle + actions）；`count` 参数化（默认 6 = grid/InstanceArea，grouped 每组 2） | 工作台实例网格 |
| `GroupedProjectsSkeleton`（instance-area） | `GroupedProjectsList` 批 J + 批 L（每组 section `overflow-hidden rounded-lg border border-neutral-line bg-surface-raised`（两端统一带 bg 边框，2026-08-03 撤销决策 38 移动无边框）；根 `px-3 py-3` 两端统一）：项目名行 `flex items-center gap-2 pl-3 pr-2`（2026-08-03 mirror 真实名行两端统一 padding，撤销批 L px-2）= 图标占位 `size-5`（批 K）+ 项目名条 `h-6 w-1/3` 对齐 `text-base font-semibold` 行盒 24px + › 占位 `size-5` + ⋯ 占位 `size-9` 对齐 ActionMenu trigger；**无实例区小标题**（折叠废弃，决策 33）；实例区 `-mt-2` 包 `CardGridSkeleton plain` 每组 3 卡 = carousel 一页；`CardGridSkeleton` plain 去 `divide-y`、占位卡 `left-15`（两端 inset）分割线 mirror 真实） | grouped 视图加载 |
