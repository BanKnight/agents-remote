...
- **card**：Project/Agent/Terminal 卡片。`surface-raised` + `rounded-xl`（20px）+ `neutral-line` 边框 + padding `lg`（16px）。Agent card 可用 `2xl`（24px）。卡片服务可扫读性，不为 metadata 牺牲首屏密度。InstanceCard 采用微信朋友圈式头像布局：左侧 marker 头像（`lg`=h-9 w-9=36px，`items-start` 上下置顶）独占一列 + 右侧内容区竖排（title / subtitle / meta），close 按钮落在 meta 行右侧（`ml-auto`）。状态圆点仍叠加 marker 右上角（`-right-1 -top-1`，4px 偏移不依赖 marker 尺寸）。marker 尺寸：card 用 `lg`（36px）/ table 与紧凑 header 用 `sm`（28px）。
