...
- **list（ListGroup + ListRow，全局列表契约）**：iOS Files 范式——**列表样式由列表性质决定，不由视口决定**。会滚动/内容多的「内容列表」（Files 文件列表、Git 改动列表）一律用 **plain** 连续行：两端一致 `divide-y divide-neutral-line/40`（Tailwind v4 实现 = 除末行外每行底部 1px separator，选择器 `> :not(:last-child)`，**非每行 gap**），无外框/圆角/卡片，贴外部 `p-3`。plain 比每行圆角卡片 + gap 更紧凑（行间无线间距），但行高仍 `px-3 py-2.5` ≈50px 保证触摸/可读。**短/固定/分组列表**（未来 Settings 式选项）才用 grouped（圆角卡片 + `mx-3`，本契约预留，当前无调用点）。**行 `ListRow`**：去 `rounded-xl`、去独立 `raised` 背景（连续行共享外部底）；非 selected 用 `hover:bg-on-surface/5`；selected 用纯 `bg-primary/10`、**去 border**——连续行里 `border-primary/60` 与行间 separator 打架，是对 `selected-row` 通用契约的连续行特化覆盖。**`divide-y` 硬约束**：`ListRow` 必须是 `ListGroup` 的**直接子**（`.map` + `key`），中间不得包 `div`/`Fragment`，否则 `> :not(:last-child)` 选择器失效、separator 消失。**边界**：`card`（可独立摆放的实体，网格有 gap 天然合理）、`nav-item`（导航行语义）不在本契约内；本契约只管**同质内容行的连续序列**。
