...
- **navigation-drawer**（移动端项目侧边栏，`mobile-project-drawer.tsx`）：**Reddit 式 push 整页平移抽屉**（2026-08-17 从 Radix `Dialog` 覆盖式改造，同日二次修正：用户反馈「整个页面被推（含 header tab 带）+ 被推页面保持视口宽不被压缩，仅整体往右平移」）。drawer = `MobileProjectWorkbench` 里 absolute 左侧**静态常驻**层（`<aside>` 非 portal、宽度恒 `min(88vw,340px)`、自身无开合过渡），开合由**页面容器**（header tab 带 + 内容作为一个整体）`translate-x-[min(88vw,340px)]` ↔ `translate-x-0` 平移（`transition-transform duration-300 ease-in-out`）：展开 = 页面滑开露出 drawer、右侧溢出被外层 `overflow-hidden` 裁掉；transform 不改 layout 宽度，**页面内容零重排零压缩**（「保持大小、仅被往右推」）。**关闭交互**（原 Radix dismissable 职责的替代）：点被推页面（父级透明拦截层 `absolute inset-0 z-10`，无视觉、关闭时不渲染）+ Esc（组件内 `useEffect` 手动监听 window keydown）。**尺寸**：宽度 `min(88vw,340px)`（Material navigation-drawer：小屏 ≥88% 宽、上限 340px），右缘 `rounded-r-2xl`（16px，Shape 档 `2xl`）。**行模型**：段导航横向 tab 行（复用 `TabButton` + `overflow-x-auto` 可横滚，active `bg-primary/10 text-primary` nav-item token，对齐桌面左栏 middle tab bar）。**结构**：顶部返回入口 A（离开项目回列表）+ 项目名 + 右上角按段新建入口；段导航（7 段横向 tab）+ 段主体（总览=会话列表+新建入口/历史=HistoryList/文件=FilesLeftPanel 文件树·Git=GitChangesList 变更列表（点文件关 drawer 开 tab / diff tab，文件·Git 并入 tab 带）/页面·Wiki=PagesPanel/WikiPanel/插件=PluginsPanel）。**DOM 常驻**：drawer 永在 DOM（absolute 静态层），开合只动页面 transform。**safe-area**：顶部避让由 main 统一 `pt-[var(--shell-safe-area-top)]` 承担（drawer 与页面同层）；底部 `pb-[env(safe-area-inset-bottom)]` 单点避让（frontend-notes §1）。
