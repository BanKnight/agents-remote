...
- [screenshots](./screenshots/) — 保存 prototype HTML 页面的浏览器渲染截图，用于评审和对齐布局。
...
- [git.html](./git.html) — 展示 Project Git 的只读 status/diff inspection 体验：桌面端变更列表 + unified diff，移动端直接二级 Git 列表保留带 Back 的底部二级导航，进入 diff 详情后使用顶部返回且隐藏二级导航。
...
- [overview.html](./overview.html) — 汇总展示首页、Project 详情页、Files 页、Git 页、Terminal 页与实例详情页，便于并排检查导航模型。
