...
- **置顶分组（2026-08-06）**：global 总览**最前**新增特殊分组「置顶」（📌），收纳跨项目置顶卡片（快捷入口，**不搬移**——置顶卡片同时在「置顶分组」和「原项目分组」出现，双显示）。置顶数据**只存客户端**：`workbenchPinnedSessionsAtom`（`atomWithLocalOnlyStorage<Record<sessionId,true=置顶>>`，localStorage key=`workbenchPinnedSessions` 按会话记忆，刷新/重开保留；sessionId 残留无候选匹配即不渲染，空分组自然隐藏，无需 GC）。**无置顶卡片时分组整段不渲染**（空隐藏）。标题行 = 折叠 toggle 同款（`▾/▸` + 📌 pin 图标 `text-primary` + 「置顶」`text-base font-semibold`，`aria-expanded`，min-h-11 热区），容器同项目行 `bg-surface-raised/30 rounded-lg` raised 抬升条，折叠态复用 `workbenchProjectGroupsCollapsedAtom` 的保留哨兵 key `"__pinned__"`（localStorage 记忆）；**无 `›` 进项目、无 `⋯` 删除**（非项目，只折叠 toggle）。每张 global 总览卡片**右下角**置顶按钮（`absolute bottom-2 right-2`，镜像 ⋯ 按钮，h-9 w-9 / touch:h-11 w-11，icon h-5 w-5——2026-08-06 从 h-7/h-10 放大：桌面 36px、触屏 44px 达 DESIGN 触屏目标规范 + Apple HIG；`aria-pressed`；置顶态 icon `text-primary`，否则 muted + hover 亮）——tap 置顶/取消置顶，`stopPropagation` 双层防 portal fiber 冒泡导航。**卡片右上 ⋯ 按钮同款放大**（h-9 w-9 / touch:h-11 w-11，icon h-5 w-5）。**仅 global 总览卡片渲染**（InstanceCard 可选 props `pinned`/`onTogglePin`/`pinLabel`，缺失不渲染 → 项目 scope/纯展示卡零改动）；文字让位：title 行恒让位（给 ⋯）、subtitle/meta 行置顶时让位（给 pin），均 `pr-10 touch:pr-12`（40/48px = 按钮列 44/52（right-2 8 + h-9 36 / h-11 44）− card p-3 12，右缘 = 按钮.left − 8 间隙防误触；title 行旧 pr-6 触屏钻 ⋯ 下方 12px 一并修复）——subtitle 是卡片末行（无 meta）时与按钮垂直重叠，须与 meta 同款让位。置顶组也传 `dragAdapter` + dragRefs（桌面左栏可拖进右工作区）。
