...
| scope | 呈现 | 项目分段 |
|-------|------|---------|
| global | **最前置顶分组**（📌，空隐藏，服务端持久化）+ 项目标题行（手风琴：▾ 折叠独立按钮 + 📁 项目名点击进入 + 行内新建 Claude/Terminal/删除按钮 + › 进项目）作 section header 分割，每段下单列详细卡 | 是（`mergeProjectsWithCandidates`，含无实例空项目只显标题行） |
| project | 单列详细卡直接平铺（本项目实例，无需分段） | 否 |

- **global 卡片**（朋友圈式）：左 marker 头像（lg=h-9，上下置顶）+ 右 3 行（会话名 / 末行 output 预览 / 项目名·时间 + close 右推）。
- **项目标题行（手风琴，2026-08-06；2026-08-10 重设计）**：`[▾ 折叠独立按钮][📁 项目名 flex-1 button 点击进入项目][+Claude][+Terminal][🗑 删除][› 进项目 最右]`。**行主按钮语义 = 进入项目**（navigate `/projects/$key`，2026-08-10 由「折叠/展开」翻转：整行点击进项目，折叠不再占行主体）。**折叠/展开只由 ▾ 独立按钮触发**（2026-08-10 拆出：`aria-label` = `workbench.collapseProjectGroup`/`expandProjectGroup` 按态切换 + `aria-expanded`，chevron expanded=`M4 6l4 4 4-4`（▾ 下）/ collapsed=`M6 4l4 4-4 4`（▸ 右）；折叠态左 ▸ + 右 › 两个右箭头分居行两端可读），折叠态存 `workbenchProjectGroupsCollapsedAtom`（`atomWithLocalOnlyStorage<Record<projectName,true=折叠>>`，localStorage 按项目记忆，刷新/重开保留）。**右侧独立 `›` 按钮进项目**（navigate `/projects/$key`，`aria-label=workbench.enterProject`，h-7 w-7 / touch:h-10 w-10，最右视觉锚）。**⋯ 菜单下线（2026-08-10）**：原菜单项展开为行内 icon 独立按钮——+Claude（anthropic icon）/ +Terminal（terminal icon）行级 `useCreateSession(projectName)`（name prompt → 创建 API → navigate `/projects/$key/session/$id` + invalidate，不需进项目直接建；空项目行同样可建）+ 删除（trash icon destructive，`deleteProject` + useConfirm confirm）。**桌面右键随 ⋯ 下线（2026-08-10）**——行内已暴露全部操作。**标题行容器 `bg-surface-raised/30 rounded-lg` raised 抬升条**（2026-08-06 优化：与下方透明卡片区形成分组边界，折叠/展开态均保持）——卡片连续单列平铺，组内非首卡由 InstanceCard `topSeparator` 画 inset 分割线（两端统一 left-15=60px）。
...
- **项目标题行行内新建/删除（2026-08-06 续七菜单，2026-08-10 展开为行内按钮）**：原 ⋯ 菜单的「新建 Claude（claude2）+ 新建 Terminal」前置 +「删除项目」最右——**2026-08-10 ⋯ 菜单下线，三项展开为行内 icon 独立按钮**。对齐项目 `CreateSessionBar` 新建项，**不需进具体项目即可在总览直接建实例**。实现 = 行级 `ProjectRowActions` 组件（`global-projects-overview.tsx`，原 `ProjectRowActionMenu` 改名去 ActionMenu），内部 `useCreateSession(projectName)`（name prompt → `createAgentSession`/`createTerminalSession` → navigate `/projects/$key/session/$id` + invalidate，与项目内 `CreateSessionBar` 行为一致：创建即进入会话工作台）。hook 在组件内调用（`groups.map` 内联调 hook 违反 hooks 规则）——标题行 `<ProjectRowActions>` 放 `📁 名 button` 与 `›` 之间，props = `{ projectName, onDelete, deletePending }`（去 `ctx.pointFor/close`——右键随 ⋯ 下线）。按钮项：Claude（anthropic icon，`workbench.createClaude2`）+ Terminal（terminal icon，`workbench.createTerminal`）default 前置、删除（trash icon destructive 静息 muted / hover `text-error` + `bg-error/10`）最右；`isCreating` 禁用新建项、`deletePending` 禁用删除项。**空项目行同样可建**（最需要直接建第一个实例）；置顶分组标题行无 ⋯ 不涉及。不加 Codex（项目里 `CreateSessionBar` 也没有，`workbench.createCodex` key 无消费点）。
- **空项目**：global 含空项目（只显标题行，可进/可删）；project scope 无分段概念。空项目行**无折叠 chevron**（无可折叠内容），主区为 📁 名 button（`flex-1`，`onClick` 进入项目，`min-h-11` 热区 ≥44px），仍保留 `+Claude`/`+Terminal`/`🗑` 行内按钮 + `›` 进项目。
