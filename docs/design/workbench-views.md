...
| scope | 呈现 | 项目分段 |
|-------|------|---------|
| global | **最前置顶分组**（📌，空隐藏，仅客户端）+ 项目标题行（手风琴：▾/▸ 折叠 toggle + 📁 项目名 + › 进项目 + ⋯ 菜单（新建 Claude/Terminal + 删除））作 section header 分割，每段下单列详细卡 | 是（`mergeProjectsWithCandidates`，含无实例空项目只显标题行） |
