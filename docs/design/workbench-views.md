...
- 2026-08-10（续七）：terminal 亮色 256 色源头精准映射 Solarized Light（`extendedAnsi`）。用户问「2566 色没有原生方案可以做到吗」——调研发现 xterm.js 有公开 API `ITheme.extendedAnsi?: string[]`（typings:415「ANSI extended colors (16-255)」），`ThemeService._setTheme`（ThemeService.ts:129-134）按下标 `i` 写入 `colors.ansi[i+16]`，`parseColor("")` 抛错走 fallback 保留默认（**空串 = 保留默认**，契合稀疏覆盖）。**续六 `#AFD7FF` 误记修正**：`38;5;153` 实为 `rgb(135,215,255)=#87D7FF`（xterm 256 cube 值 `[0,95,135,175,215,255]`，153=216-cube 第 (2,4,5) 档 → 135/215/255）。Solarized 官方只有 16 精确色、无现成 256 cube，用户拍板**精准映射 3 索引**（非全量 240 档 CIELAB nearest-color——避免 htop/vim 渐变退化）：`153` → Solarized blue `#268bd2`、`220` → yellow `#b58900`、`246` → base01 `#586e75`。仅亮色挂（数组长度 231 = 覆盖到 246，其余空串保留默认）；暗色 `extendedAnsi: undefined` 零变化。与 `minimumContrastRatio` 分工：前者源头替换 256 色（精确可控）、后者兜底 truecolor（`38;2;r;g;b` 绕过调色板 extendedAnsi 管不到），两者并存。文档同步：DESIGN.md Terminal theme 节追加 `extendedAnsi` 段落 + 修正续六 `#AFD7FF`→`#87D7FF`。
