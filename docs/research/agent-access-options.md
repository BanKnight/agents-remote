...
- 证据分级收口 artifact：`.workflow/changes/research-agent-access-options/artifacts/evidence-grading.md`。
...
- **第一轮真实可用链路与最终版目标必须区分**：roadmap v0.1-v0.3 可先用 terminal passthrough 跑通真实 Claude/Codex CLI；最终版才讨论 provider-native thread/turn/event、原生 Agent UI 与 capability 扩展。
...
### 第一轮真实可用链路（roadmap v0.1-v0.3）：终端 passthrough
...
provider-specific metadata 在 session bootstrap / agent registry / backend runner 层生成或保留：

- `/home/deploy/repos/hapi/cli/src/agent/sessionFactory.ts`
- `/home/deploy/repos/hapi/cli/src/agent/sessionBase.ts`
- `/home/deploy/repos/hapi/cli/src/agent/AgentRegistry.ts`
- `/home/deploy/repos/hapi/cli/src/agent/runners/runAgentSession.ts`
- `/home/deploy/repos/hapi/cli/src/agent/backends/acp/AcpSdkBackend.ts`
...
roadmap v0.1-v0.3 使用真实 CLI passthrough，因此 provider 自身支持的 slash commands、skills、plugins、autocomplete、交互式提示应由 CLI 原样处理。本项目只负责可靠转发输入输出，不重新实现这些能力。

这是第一轮真实可用链路选择 terminal passthrough 的重要原因：它能避免早期原生 UI 因未覆盖 provider-specific 能力而造成能力遗漏。
...
### 给 `design-session-runtime-boundaries`
