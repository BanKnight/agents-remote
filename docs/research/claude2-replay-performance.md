...
3. **新主因**：≥86% 落在「服务端发」+「tunnel/网络传」之间，客户端侧分不开，需服务端 flush 计时进一步拆分（区分服务端 CPU vs tunnel/网络）。
...
| A：尾部窗口分页（先发最近 N 条，向上滚回填） | 客户端+服务端 | 与历史长度解耦，**实测后确认为真正杠杆**（主因在传输） | 下一步（需服务端配合，待讨论） |
| C：服务端流式 + 背压 | 服务端 | 去 event-loop 阻塞 + 去瞬时 burst | 后续 |
| D：压缩感知折叠（复用 compact-block，默认只渲染末次 compact 之后） | 客户端 | 语义减负 | 可选 |
...
4. **服务端分页（A）+ flush 计时** ⬅️ 当前下一步：加 `history` flush 墙钟（区分 flush CPU vs tunnel/网络），再设计尾部分页契约（先发最近 N 条、向上滚回填）。详见讨论。
...
**验收目标**（同方法、同样本复测）：
- 客户端首屏可见 **< 2s**（与会话长度无关）。⚠️ 因实测主因在传输，目标达成依赖**分页（减少传输量）**，而非派生增量。
- 接收阶段等帧 `arrival.wait` **< 500ms**（首屏尾窗送达即可，不再等全量）。
- 主线程单次阻塞任务 **< 50ms**。
- 不回归：现有 web 测试全 pass；`normalizeChatStream` / `renderChatStream` 纯函数测试不破。
