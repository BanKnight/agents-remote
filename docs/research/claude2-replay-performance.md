...
| A：尾部窗口分页（先发最近 N 条，向上滚回填） | 客户端+服务端 | 与历史长度解耦；超长会话压缩+分块仍不够时的真正杠杆 | 后续（针对 70MB+ 再评估） |
...
5. **服务端分页（A）+ flush 计时** ⬅️ 压缩+分块落地后的下一步：压缩不 scale（72MB 仍 ~13.8MB），且派生/渲染沉淀成本（normalize/commit）在超长会话会再次抬头，故仍需尾部分页（先发最近 N 条、向上滚回填）。针对 70MB+ 推进。
...
- 主线程单次阻塞任务 **< 50ms**。⚠️ 仍有 `normalize` 峰值 128.6ms、`commit` 峰值 587.8ms —— 发生在 `historyRecv` 之后异步阶段，对 12MB 首屏影响可控，但 70MB+ 会话需分页进一步降低。
- 不回归：现有 web 测试全 pass（310）；api 122 pass；`normalizeChatStream` / `renderChatStream` 纯函数测试不破。
