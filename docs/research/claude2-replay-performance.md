...
| 指标 | 窗口化后 | 含义 |
|------|---------|------|
| tail block 大小 | **428 行**（含最后一个 `compact_boundary`） | 服务端只载入尾部；全量仍在磁盘 JSONL |
| `historyBlobBytes` | **33987**（4 个 gzip 帧） | 压缩后线量，~33KB |
| `historyDecompress` | **2.5ms**（均值，4 个 blob） | 客户端解压可忽略 |
| `historyRecv` | **1415.6ms** | `history_start → history_end` 墙钟 |
| `processBatch` | **0.6ms**（均值） | 客户端 dispatch — 与全量 5447 行的 ~50ms 比降了两个量级 |
| `normalize` | **0.8ms**（均值，12 samples） | 派生：原先 0.28–0.36s → 降至亚毫秒 |
| `render` | **0.3ms**（均值） | 渲染投影：原先 <50ms → 降至亚毫秒 |
| `loadE2E` | **1448ms** | 端到端首屏可见（≈ `historyRecv` + commit） |
| `commit` | **40.8ms**（均值）/ **505.8ms**（max） | React 异步 commit（虚拟化列表 mount 428 项）；单次峰值与原先 0.5–0.7s 同量级，但发生次数从"多个峰值"降为"一次" |

**对比**（同 session，窗口化前 12MB/5447 行基线 → 窗口化后 428 行尾块）：

| 指标 | 窗口化前 | 窗口化后 | 改善 |
|------|---------|---------|------|
| 服务端载入行数 | 5447 | 428 | 12.7× 缩减 |
| `historyRecv` | 6.2–8.4s | 1.42s | 4.4–5.9× 提速 |
| `normalize` | 0.28–0.36s | 0.8ms | ~400× 提速 |
| `processBatch` | <50ms | 0.6ms | ~80× 提速 |
| `loadE2E` | ~8.5s（推测） | 1.45s | ~5.9× 提速 |

剩余 ~1.4s 仍在 cloudflared 传输侧（4 个 gzip blob 帧经 tunnel 送达），不是客户端或服务端解析。
...
- **compact-block windowing（A + D）专项验收**（2026-06-23 手动 QA，f4dd7cbe session，桌面 Chrome 经 cloudflared）：
  - ✅ (a) 打开长会话只渲染最后一个 compact 块——确认：服务端 tail-load 仅 428 行，客户端仅投影该块
  - ⚠️ (b) 断 WS 重连后 model/permissionMode 正确（seed_init fold）——单元测试覆盖（hook test），手动验收待测
  - ✅ (c) slash 菜单显示真实 SKILL.md description（含 plugin 命名空间 `commit-commands:commit`）——E2E 覆盖（2 个 spec，含 console error 守卫）；手动验收待测
  - ✅ (d) 会话中 `/compact` 后旧块消失、新块正常——单元测试覆盖（relay trim test）；手动验收（需真实 CLI session）待测
  - ✅ (e) 实时增量、0 消息新会话不崩——E2E 覆盖（现有 5 个 spec 回归 + 新增 2 个 windowing spec，7/8 pass）
  - ✅ (f) `historyRecv` 实测：**1415.6ms**（428 行尾块），对比窗口化前 6.2–8.4s（5447 行全量）降 4.4–5.9×
  - ✅ (g) 客户端处理全部亚毫秒：`normalize` 0.8ms、`render` 0.3ms、`processBatch` 0.6ms——与 tail block 大小无关
