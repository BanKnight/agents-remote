...
| 字段                                | 含义                          | 用途                                     |
| ----------------------------------- | ----------------------------- | ---------------------------------------- |
| `uuid`                              | 消息唯一标识 + **持久化分类标记** | replay / reconnect 的尾部补齐依据。**有 `uuid` 的消息是对话树的持久节点（写入 JSONL）；无 `uuid` 的消息是瞬时信号（仅 stdout，不写入 JSONL）。已知例外：`mode`（无 `uuid` 但写入 JSONL）、`system.init`（有 `uuid` 但不写入 JSONL）。** |
...
**出现位置**：仅 JSONL 文件。CLI stdout 不输出此类型。
...
**`uuid` 作为持久化分类标记**：有 `uuid` 的消息是对话树的持久节点（写入 JSONL）；无 `uuid` 的消息是瞬时信号（仅 stdout，不写入 JSONL）。`mode` 是目前已知的唯一例外——无 `uuid` 但写入 JSONL。
