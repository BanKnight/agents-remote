...
- [Claude2 客户端调试开关 Runbook](./runbooks/claude-client-debugging.md) — 浏览器运行时调试开关（socket 日志、调试按钮，均默认关闭）的控制范围与切换方法。
...
- [Claude2 Replay 性能与验收基线](./research/claude-replay-performance.md) — 长会话打开慢的分析依据：数据流成本模型、实测数字（客户端已排除，主因在传输）、实施路径与验收标准。
