...
- [Claude 进程模型与消息回放设计](./design/message-replay.md) — Claude 直拉 CLI（`Bun.spawn`，非 tmux）+ JSONL history / 内存 live 双缓冲 relay + 单一 WS 流的管线设计，含 system.init/turn 边界与 reconnect/API 重启时序。
...
- [Claude Replay 性能与验收基线](./research/claude-replay-performance.md) — 长会话打开慢的分析依据：数据流成本模型、实测数字（客户端已排除，主因在传输）、实施路径与验收标准。
