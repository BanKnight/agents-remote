...
export function ShellMobileBottomNavigation({
  ...
  return (
    ...
    >
      <div
        className={`mx-auto grid w-fit max-w-full gap-1 rounded-2xl border border-on-surface/10 bg-surface-raised/40 px-2 py-2 shadow-2xl shadow-black/40 backdrop-blur-xl backdrop-saturate-150 ${colsClass}`}
      >
        {children}
      </div>
    </nav>
...
export function ProjectShellNavigation({
  ...
  return (
    <>
      <Link
        className="mb-4 inline-flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold text-on-surface-muted hover:text-primary"
        to="/"
      >
