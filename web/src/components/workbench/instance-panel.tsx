 ...
 *
 * 复用 ClaudeChat 的 embedded 模式：跳过 ShellLayout/sidebar，直接渲染聊天主体
 *（ChatHeader + AssistantRuntimeProvider + thread + composer），由 WorkbenchShell 提供外壳。
 * AssistantRuntimeProvider 在 ClaudeChat 内部，每面板独立 runtime，天然支持多实例（Stage 4）。
 *
...
/**
 * 非 claude agent（codex/claude）实例面板，复用 SessionDetail embedded 模式
 ...
 * 由 WorkbenchShell 提供外壳。claude 有专用 ChatPanel（assistant-ui chat 体验），其余
 * agent 走此 stream 面板。
 *
