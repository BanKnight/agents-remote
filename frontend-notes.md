...
- **条件规则**（2026-08-04：pr 左推 → capsule 自身收窄居中 → calc 连续自适应 → 居中拉满 → w-full 撑到上限）：capsule `group-has-[.mobile-fab]:w-full group-has-[.mobile-fab]:max-w-[calc(100vw-9.5rem)]`（9.5rem=152px=2×(FAB 占位 68px+间隙 8px)）+ `mx-auto` 居中 + item `group-has-[.mobile-fab]:gap-1`/`px-1` 收紧 = `main.group :has(.mobile-fab)` 时胶囊**自身收窄到「视口−两侧对称让位」居中**（间隙 8px 不贴死、中心恒居中、各视口自适应：390w→238/375w→223/360w→208；两侧对称留白，FAB 落右侧），中心恒定不左推；无 FAB 页 capsule `w-fit max-w-full` 按内容收缩不撑满（留两侧留白舒展）。旧 `group-has-[.mobile-fab]:pr-[4.5rem]`（nav 右 padding 72px 把胶囊左推 ~30px）已弃用——FAB 出现/消失时 capsule 整体平移、位置突兀。
...
1. **让位 = 被让位元素自身收窄，不是推兄弟元素**：capsule（被让位元素）用 `group-has-[.marker]:max-w-[calc(100vw-9.5rem)]` 自身收窄（视口−两侧对称让位，间隙恒定）+ `mx-auto` 居中，让位 marker class 挂条件对象（FAB）底座。`group` 加在**实际渲染该元素的祖先**上（先确认哪层 main 是真身，别凭目录猜）。**收窄居中**（capsule 中心恒定、宽度随有无 FAB 变）优于 **padding 左推**（capsule 整体平移、FAB 出现/消失时位置突兀）——两者都是 `:has` 条件让位，但收窄居中保位置恒定（用户接受宽度变化、不接受位置跳）。
...
- **2026-08-04 优化 commit `c40522f`**（pr 左推 → capsule 自身收窄居中）：撤销 `pr-[4.5rem]`，改 capsule `group-has-[.mobile-fab]:max-w-[13rem]`（208px）+ `mx-auto` 居中 + item `group-has-[.mobile-fab]:gap-1`/`px-1` 收紧。探针实测（3 视口）：390w capsule width=208 center=195（≡视口中心 195，**不左推**）right=299 ≤ fab.left=322（间隙 23）；375w right=292 ≤ 307（间隙 15）；360w right=284 ≤ 292（间隙 8）；4 label（Projects/Files/Skills/Settings）无截断；/files 无 FAB capsule 撑满（390w=366）居中；**切换 `/` ↔ `/files` capsule.center 三视口全恒定（195/188/180 ===）= 零跳变**（核心价值；旧 pr 左推时 capsule 整体平移 ~30px、出现/消失突兀）。
- **2026-08-04 放宽 commit `f0476c3`**（13rem 固定 → calc 连续自适应）：用户反馈有 FAB 页 capsule 208 偏窄、390w 间隙 23 浪费。根因——固定 `max-w` 被最小视口（360w）卡死，大屏间隙浪费。改 `max-w-[calc(100vw-9.5rem)]`（9.5rem=152px=2×(FAB 占位 4.25rem+间隙 0.5rem)），capsule 宽=视口−两侧对称让位，间隙恒 8px、各视口自适应（390w→238/375w→223/360w→208）。**优于离散分档**：几何等价（间隙恒定、各视口贴近 FAB）但一行连续自适应、无 `min-[375px]:`/`min-[390px]:` variant 排序风险；`calc(100vw-9.5rem)` 在 arbitrary value 里运算符无空格，Tailwind v4 正确生成 `max-width:calc(100vw - 9.5rem)`。
- **2026-08-04 居中拉满 commit `6b78b7b`**（calc 间隙 8 → 2）：用户反馈 238 仍偏窄。居中约束下物理上限 = 视口 − 2×(FAB 占位 68 + 最小间隙)，390w 上限 ~254。将 `calc(100vw-9.5rem)`（间隙 8）→ `calc(100vw-8.75rem)`（8.75rem=140px=2×(FAB 68+间隙 2)），capsule 紧贴 FAB（间隙 2px），390w 250、375w 235、360w 220（各 +12px）。**居中拉满已是物理上限**——再宽只能放弃居中（capsule 偏左收回左侧留白，但 `/`↔`/files` 切换 center 跳变 ~30px = 旧 pr 位置突兀部分回归，用户已否决）。零跳变核心保留。
